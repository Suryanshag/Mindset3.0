/**
 * Sprint UI-2a verification capture: 3 routes touched by the density
 * pass + notifications fix.
 *
 * Run:
 *   AUDIT_PASSWORD='<password>' npx tsx tools/screenshot-sprint-ui2a-verification.ts
 */
...
const ROUTES: RouteSpec[] = [
  ...
  { route: '/user/sessions', filename: 'user__sessions.png' },
  { route: '/user/notifications', filename: 'user__notifications.png' },
]
...
const OUT_DIR = path.join('docs/ui-audit', `sprint-ui2a-verification-${TIMESTAMP}`)
...
async function main() {
  ...
  const lines: string[] = []
  lines.push(`# Sprint UI-2a — Verification capture — ${TIMESTAMP}`)
  lines.push('')
