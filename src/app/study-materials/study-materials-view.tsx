          ...
          <div class="block-blog-listing__content">
            ...
            <div class="block-blog-listing__main">
              <div class="block-blog-listing__posts" data-posts-container>
                <div class="ms-skel-grid" aria-busy="true" aria-label="Loading materials">
                  ${Array.from({ length: 6 }).map(() => `
                    <div class="ms-skel-card">
                      <div class="ms-skel ms-skel-card__media"></div>
...
function buildMaterialDetail(material: StudyMaterial): string {
  ...
  const ctaButton = isFree
    ? `<a href="/api/user/ebooks/${material.id}/serve" target="_blank" rel="noopener noreferrer" class="doctor-detail__book-btn">
        Download Free
...
export default function StudyMaterialsView({ initialDetailId }: { initialDetailId?: string }) {
  const containerRef = useRef<HTMLDivElement>(null);
  const postsRef = useRef<HTMLElement | null>(null);
  const [materials, setMaterials] = useState<StudyMaterial[]>([]);
  const [view, setView] = useState<"list" | "detail">(initialDetailId ? "detail" : "list");
  ...
  const detailIdRef = useRef<string | null>(initialDetailId ?? null);
  ...
  const openDetailState = (id: string) => {
  ...
  const { openDetail, closeDetail } = useDetailRoute({
    ...
    onOpen: openDetailState,
    onClose: closeDetailState,
  });

  useEffect(() => {
    showDetailRef.current = openDetail;
    goBackRef.current = closeDetail;
  }, [openDetail, closeDetail, materials]);

  // Deep-link: resolve the requested material once the list has loaded. If the
  // id isn't in the loaded list (stale/shared link), fall back to the listing.
  useEffect(() => {
    if (view !== "detail" || !detailIdRef.current || selected) return;
    const found = materials.find((m) => m.id === detailIdRef.current);
    if (found) setSelected(found);
    else if (materials.length > 0 || fetchError) goBackRef.current();
  }, [materials, view, selected, fetchError]);
