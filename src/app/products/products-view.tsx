...
export default function ProductsView({ initialDetailId }: { initialDetailId?: string }) {
  ...
  const openProductState = (id: string) => {
  ...
  const { openDetail, closeDetail } = useDetailRoute({
    basePath: "/products",
    onOpen: openProductState,
    onClose: closeProductState,
  });

  useEffect(() => {
    showProductRef.current = openDetail;
    goBackRef.current = closeDetail;
  }, [openDetail, closeDetail, products]);

  // Deep-link: resolve the requested product once the list has loaded. If the
  // id isn't in the loaded list (stale/shared link), fall back to the listing.
  useEffect(() => {
    if (view !== "detail" || !detailIdRef.current || selectedProduct) return;
    const found = products.find((p) => p.id === detailIdRef.current);
    if (found) setSelectedProduct(found);
    else if (products.length > 0 || fetchError) goBackRef.current();
  }, [products, view, selectedProduct, fetchError]);
