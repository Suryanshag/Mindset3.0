...
export default function WorkshopsView({ initialDetailId }: { initialDetailId?: string }) {
  ...
  // Which workshop id the URL is pointing at — survives the list still loading
  // so a deep-linked /workshops/<id> resolves once the fetch completes.
  ...
  }, [openDetail, closeDetail, workshops]);

  // Deep-link: open the requested detail once the list has loaded. If the id
  // isn't in the loaded list (stale/shared link), fall back to the listing.
  useEffect(() => {
    if (view !== "detail" || !detailIdRef.current || selected) return;
    const found = workshops.find((w) => w.id === detailIdRef.current);
    if (found) setSelected(found);
    else if (ready || fetchError) goBackRef.current();
  }, [workshops, view, selected, ready, fetchError]);
  ...
  useLayoutEffect(() => {
    ...
    realCards.forEach((card) => {
      card.addEventListener("click", (e) => {
        ...
        if (postId?.startsWith("w-")) showDetailRef.current(postId.slice(2));
      });
    });
